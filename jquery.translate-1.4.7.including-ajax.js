                ...
                if (o.altAndVal && (nodeName === "IMG" || type === "image")) text = e.attr("alt");
                    ...
                    if (!hasChildNode) text = e.text();
                        ...
                        if (hasTextNode) { //remove child nodes from jq
                            //remove scripts:
                            text = e.html(); /*jslint skipLines*/
                            text = o.stripScripts ? text.replace(/<script[^>]*>([\s\S]*?)<\/script>/gi, "") : text; /*jslint skipLinesEnd*/
    ...
    T.prototype = {
        ...
        _process: function() {
            ...
            while ((lastpos = this.rawTranslation.lastIndexOf("</div>", i)) > -1) {
                ...
                i = lastpos - 1;
                subst = this.rawTranslation.substr(0, i + 1); /*jslint skipLines*/
                divst = subst.match(/<div[> ]/gi);
                divcl = subst.match(/<\/div>/gi); /*jslint skipLinesEnd*/
...
/*
    ...
    $.translate.extend({
        ...
        _defaults: defaults,
        //base defaults used by other components as well //TODO
        ...
        truncate: function(text, limit) {
            ...
            (m4 = />(?![^>]*<)/.exec(t)) ? (
            m3.index > m4.index ? t.substring(0, m3.index + 1) : t.substring(0, m4.index + 1)) : t.substring(0, m3.index + 1)) : t) : t) : t.substring(0, m1.index);
        },
        ...
        getLanguages: function(a, b) {
            ...
            if (filterArg) if (filterArg.call) //if it's a filter function
        ...
        getBranding: function() {
            if (typeof console != "undefined") console.log("$.translate.getBranding() IS DEPRECATED! PLEASE REMOVE IT FROM YOUR CODE!");
            return $();
        },
...
(function($) {
    ...
    $.translate.extend({
        ...
        setLangAttr: function(e, to, o) {
            if (!o || o.setLangAttr) e.attr((!o || o.setLangAttr === True) ? "lang" : o.setLangAttr, to);
        },
...
function localize_languages(browser_lang, popup_id) {
    var language_nodes = jQuery('#translate_popup a').get(),
        llangs = [],
        // array that holds localized language names
        i;
